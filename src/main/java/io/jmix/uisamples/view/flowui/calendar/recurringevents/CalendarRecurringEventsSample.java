...
import static io.jmix.fullcalendarflowui.component.model.DayOfWeek.*;

@ViewController(id = "calendar-recurring-events")
@ViewDescriptor(path = "calendar-recurring-events.xml")
public class CalendarRecurringEventsSample extends StandardView {
    ...
    public void onCalendarEventDrop(final EventDropEvent event) {
        ...
        notifications.show("Event with \"" + calendarEvent.getId() + "\" ID is moved");
    }
    ...
    public void onCalendarEventResize(final EventResizeEvent event) {
        ...
        notifications.show("Event with \"" + calendarEvent.getId() + "\" ID is resized");
    }
